...
- Keep-alive watchdog (`modules/tracking-watchdog`): native 2-min exact-alarm chain + 15-min WorkManager backstop. While the app is killed it ONLY uploads GPS fixes natively (no FGS/task revival — the old headless revival caused repeated background ANR dialogs on HiOS and was removed). Full tracking restores when the app is reopened. Armed on tracking start, disarmed on logout. Force stop is unrecoverable by Android design. See `.agents/memory/native-tracking-revival.md`.
